...
const defaultAddr = ":50052"

type server struct {
	riftofog.UnimplementedEventServiceServer
}
...
func (s *server) ExecuteAction(ctx context.Context, in *riftofog.ActionRequest) (*riftofog.ActionResult, error) {
	...
	log.Printf("[event] ExecuteAction action_key=%s", req.ActionKey)
	out := services.ExecuteEventAction(req)
	if out != nil {
		log.Printf("[event] result success=%v result_text=%q", out.Success, out.ResultText)
	}
...
func main() {
	addr := os.Getenv("EVENT_GRPC_ADDR")
	if addr == "" {
		addr = defaultAddr
	}
	lis, err := net.Listen("tcp", addr)
	if err != nil {
		log.Fatalf("event listen: %v", err)
	}
	srv := grpc.NewServer()
	riftofog.RegisterEventServiceServer(srv, &server{})
	reflection.Register(srv)
	log.Printf("[event] gRPC listening on %s", addr)
